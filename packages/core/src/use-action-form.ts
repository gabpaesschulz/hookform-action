  ...
  const executeAction = useCallback(
    async (data: TFieldValues | FormData, isFormDataSubmission: boolean) => {
      ...
      if (hasOptimistic && resolvedOptimisticData && !(data instanceof FormData)) {
        const optimisticResult = resolvedOptimisticData(confirmedOptimisticRef.current, data as TFieldValues);
      ...
      try {
        ...
        if (fieldErrors && Object.keys(fieldErrors).length > 0) {
          ...
          if (hasOptimistic && resolvedOptimisticData && !(data instanceof FormData)) {
            confirmedOptimisticRef.current = resolvedOptimisticData(confirmedOptimisticRef.current, data as TFieldValues);
          }
