...
import type { ActionFormState, SubmissionRecord } from "next-action-form";
...
/**
 * `<FormDevTool />` – Floating debug panel for next-action-form.
 *
...
export function FormDevTool<TFieldValues extends FieldValues = FieldValues>({
  ...
  const actionFormState = (control as any)?._actionFormState as ActionFormState<unknown> | undefined;
  const submissionHistory = (control as any)?._submissionHistory as SubmissionRecord<unknown>[] | undefined;
      ...
      {formState.errors && Object.keys(formState.errors).length > 0 ? (
        Object.entries(formState.errors).map(([key, error]) => (
          <div key={key} style={kvRow}>
            ...
            <span style={{ ...valueStyle, color: COLORS.error }}>{(error as any)?.message ?? "Error"}</span>
          </div>
        ))
      ) : (
  ...
  return (
    <>
      {toggleButton}
      <div style={getPanelStyles(position)} role="dialog" aria-label="Form DevTools">
        {/* Header */}
